...
Bumpic.sln
├── src/
│   ├── Bumpic.Domain/         # 领域层 - 聚合根、实体、领域事件
│   ├── Bumpic.Infrastructure/ # 基础设施层 - EF配置、仓储接口、仓储实现
│   └── Bumpic.Web/           # 表现层 - API、应用服务
└── test/                            # 测试项目
    ├── Bumpic.Domain.UnitTests/         # 领域层测试项目
    ├── Bumpic.Infrastructure.UnitTests/ # 基础设施层测试项目
    └── Bumpic.Web.UnitTests/           # 表现层测试项目
```
...
### 文件组织
- **聚合根** → `src/Bumpic.Domain/AggregatesModel/{AggregateFolder}/`
- **领域事件** → `src/Bumpic.Domain/DomainEvents/`
- **仓储** → `src/Bumpic.Infrastructure/Repositories/`
- **实体配置** → `src/Bumpic.Infrastructure/EntityConfigurations/`
- **命令与命令处理器** → `src/Bumpic.Web/Application/Commands/`
- **查询与查询处理器** → `src/Bumpic.Web/Application/Queries/`
- **API端点** → `src/Bumpic.Web/Endpoints/`
- **领域事件处理器** → `src/Bumpic.Web/Application/DomainEventHandlers/`
- **集成事件** → `src/Bumpic.Web/Application/IntegrationEvents/`
- **集成事件转换器** → `src/Bumpic.Web/Application/IntegrationEventConverters/`
- **集成事件处理器** → `src/Bumpic.Web/Application/IntegrationEventHandlers/`
...
**Web层** (`src/Bumpic.Web/GlobalUsings.cs`):
...
**Infrastructure层** (`src/Bumpic.Infrastructure/GlobalUsings.cs`):
...
**Domain层** (`src/Bumpic.Domain/GlobalUsings.cs`):
...
**查询处理器**:
```csharp
using Bumpic.Domain.AggregatesModel.{AggregateFolder};
...
```csharp
using Bumpic.Domain.AggregatesModel.{AggregateFolder};
using Bumpic.Web.Application.Commands.{FeatureFolder};
using Bumpic.Web.Application.Queries.{FeatureFolder};
```
