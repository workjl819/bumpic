---
applyTo: "src/Bumpic.Infrastructure/Repositories/*.cs"
---
...
- 仓储接口和实现应放置在 `src/Bumpic.Infrastructure/Repositories/` 目录下。
- 文件名格式为 `{AggregateName}Repository.cs`。

## 代码示例

**文件**: `src/Bumpic.Infrastructure/Repositories/AdminUserRepository.cs`

```csharp
using Bumpic.Domain.AggregatesModel.AdminUserAggregate;
